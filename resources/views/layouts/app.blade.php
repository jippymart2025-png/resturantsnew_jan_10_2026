...
<head>
    ...
    <style>
        ...
        /* Header */
        .order-popup-header {
            background: linear-gradient(135deg, var(--theme-color), #ff8a65);
            color: #fff;
            padding: 16px 22px;
            border-bottom: none;
        }
...
<div class="modal fade" id="new_order_popup"
    ...
    <div class="modal-dialog modal-lg modal-dialog-centered">
        <div class="modal-content order-popup">
            ...
            <!-- Header -->
            <div class="modal-header order-popup-header">
            ...
            <!-- Body -->
            <div class="modal-body order-popup-body">

                <div class="order-meta">
                    <div><strong>Order ID:</strong> <span id="popup_order_id"></span></div>
                    <div><strong>Date:</strong> <span id="popup_order_date"></span></div>
                </div>

                <hr>

                <div class="order-section">
                    <p><i class="fa fa-user"></i> <strong>Customer:</strong> <span id="popup_customer"></span></p>
                    <p><i class="fa fa-phone"></i> <strong>Phone:</strong> <span id="popup_phone"></span></p>
                    <p><i class="fa fa-map-marker"></i> <strong>Address:</strong> <span id="popup_address"></span></p>
                </div>

                <hr>

                <div class="order-section">
                    <label class="mb-2"><strong>Estimated Preparation Time</strong></label>
                    <select id="popup_estimated_input" class="form-control estimated-select">
                ...
                <div class="order-summary">
                    <div><strong>Payment:</strong> <span id="popup_payment"></span></div>
                </div>

                <hr>

                <h6 class="mb-2">🧾 Items</h6>
                <ul id="popup_items" class="order-items"></ul>

            </div>

            <!-- Footer -->
            <div class="modal-footer order-popup-footer">
                <button class="btn btn-outline-danger px-4" id="rejectOrderBtn">
                    Reject
                </button>
                <button class="btn btn-success px-4" id="acceptOrderBtn">
                    Accept Order
                </button>
            </div>
        ...
        function startListener() {
            setInterval(() => {
                $.get(ENDPOINT_LATEST, function (res) {
                    console.log("LATEST ORDER RESPONSE:", res);
                    const newest = res.latest_id ?? "";


                    // First load: store latest, no popup
                    if (!lastOrderId && newest) {
                        lastOrderId = newest;
                    ...
                    if (newest && newest !== lastOrderId) {
                        console.log("NEW ORDER DETECTED:", newest);
                        ...
                        $.get(ENDPOINT_ORDER(newest), function (order) {
                            ...
                            if (
                                ...
                                showNewOrderPopup(order); // ✅ THIS WAS MISSING
                            }
...
<script>
    function showNewOrderPopup(order) {
        // Reset estimated time dropdown on every popup open
        $('#popup_estimated_input').val('');


        if (!order.status || order.status.toLowerCase() !== 'order placed') return;


        $('#popup_order_id').text(order.order_id);
        $('#popup_order_date').text(order.order_date);
        $('#popup_customer').text(order.customer.name);
        $('#popup_phone').text(order.customer.phone);
        $('#popup_address').text(order.address);
        $('#popup_payment').text(order.payment_type);


        const items = $('#popup_items');
        items.empty();

        let total = 0;

        order.popup_items.forEach(item => {
    ...
    `;
        ...
        items.append(`
    <li style="border-top:1px dashed #ccc;padding-top:10px;margin-top:10px;">
        <div style="display:flex;justify-content:space-between;font-size:18px;">
            <strong>Grand Total</strong>
            <strong>₹${order.popup_total}</strong>
        </div>
    </li>
`);


        $('#new_order_popup').modal({
            backdrop: 'static',
            keyboard: false,
            show: true
        });
        ...
        $('#acceptOrderBtn').off().on('click', function () {

            let estimatedTime = $('#popup_estimated_input').val();

            // If vendor did NOT enter anything → default 20 mins
            if (!estimatedTime || estimatedTime <= 0) {
                estimatedTime = 20;
            }

            updateOrderStatus(order.order_id, 'Order Accepted', estimatedTime);
        });


        $('#rejectOrderBtn').off().on('click', function () {
            updateOrderStatus(order.order_id, 'Order Rejected');
        });
    }
    function updateOrderStatus(orderId, status, estimatedTime) {
        $.ajax({
            url: `/orders/${orderId}`,
            method: 'POST',
            data: {
                _method: 'PUT',
                status: status,
                estimated_time: estimatedTime, // ✅ backend already supports this
                _token: $('meta[name="csrf-token"]').attr('content')
            },
            success: function () {
                $('#new_order_popup').modal('hide');
                location.reload();
            },
            error: function (xhr) {
